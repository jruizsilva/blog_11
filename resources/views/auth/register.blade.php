    ...
    <div class="p-4 sm:p-7">
      ...
      <div class="mt-5">
        ...
        <form action="{{ route('register.store') }}" class="grid gap-y-4" method="POST">
          @csrf
          <x-form.input id="name" name="name" :label="_('Name')" required />
          <x-form.input id="last_name" name="last_name" :label="_('Last name')" required />
          <x-form.input type="email" id="email" name="email" :label="_('Email')" required />
          <x-form.input type="password" id="password" name="password" :label="_('Password')" required />
          <x-form.input type="password" id="password_confirmation" name="password_confirmation"
            :label="_('Confirm password')" required />
